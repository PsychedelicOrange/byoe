...
{
    s_RendererSDFInternalState.clear_tex_resources.shader   = gfx_create_compute_shader("./game/shaders_built/clear_texture.comp.spv");
...
{
    gfx_destroy_compute_shader(&s_RendererSDFInternalState.clear_tex_resources.shader);
...
{
    {
        ...
        s_RendererSDFInternalState.clear_tex_resources.root_sig = gfx_create_root_signature(&set_layout_0, 1, NULL, 0);
    }
...
{
    gfx_destroy_root_signature(&s_RendererSDFInternalState.clear_tex_resources.root_sig);
...
static void renderer_internal_create_pipelines(void)
{
    ...
    s_RendererSDFInternalState.clear_tex_resources.pipeline = gfx_create_pipeline(clear_tex_pipeline_ci);
...
{
    gfx_destroy_pipeline(&s_RendererSDFInternalState.clear_tex_resources.pipeline);
...
{
    s_RendererSDFInternalState.clear_tex_resources.shader_read_view = gfx_create_texture_resource_view((gfx_resource_view_create_info){
        .resource = &s_RendererSDFInternalState.sdfscene_resources.scene_texture,
        .texture  = {
             .layer_count  = 1,
             .base_layer   = 0,
    ...
    s_RendererSDFInternalState.clear_tex_resources.table = gfx_create_descriptor_table(&s_RendererSDFInternalState.clear_tex_resources.root_sig);
    gfx_descriptor_table_entry table_entries[]           = {
    ...
    gfx_update_descriptor_table(&s_RendererSDFInternalState.clear_tex_resources.table, table_entries, ARRAY_SIZE(table_entries));
}
...
static void renderer_internal_destroy_sdf_pass_resources(void)
{
    ...
    gfx_destroy_texture_resource_view(&s_RendererSDFInternalState.clear_tex_resources.shader_read_view);
    gfx_destroy_descriptor_table(&s_RendererSDFInternalState.clear_tex_resources.table);
